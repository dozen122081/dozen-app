...
const Clock = ({
  ...
  const fetchData = async () => {
    try {
        const response = await fetch(`/api/workspace/clock?workspaceId=${workspaceId}`)
      if (response.ok) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(('/api/workspace/clock'), {
        method: "POST",
        body: JSON.stringify({ 
            date: targetDate, 
            author: userId,
  ...
  const deleteWorkspaceDate = async (noteId: string) => {
    try {
        const response = await fetch('/api/workspace/clock', {
            method: 'DELETE',
            headers: {
            ...
            body: JSON.stringify({ id: noteId, author: userId})
        });
        fetchData()
        ...
        <div className="flex flex-col gap-10 h-[100vh] w-full py-16 justify-start items-center ">
          ...
          <div className="flex flex-col gap-5 w-[40%] p-10 border-2 text-xl">
            ...
            <div className="flex flex-col h-[20vh] max-h-[50vh] overflow-auto">
              {dbDate.map((date) => (
                <div key={date.id} className="flex gap-5">
                  <input
                    type="checkbox"
                  ...
                  <button onClick={() => deleteWorkspaceDate(date.id)}>
